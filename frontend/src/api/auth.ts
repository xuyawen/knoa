...
export async function getUserList(
  ...
  if (force) invalidateDictPrefix('users:')
  const params = new URLSearchParams()
  params.set('page', String(page))
  params.set('size', String(size))
  if (role) params.set('role', role)
  if (q) params.set('q', q)
  const key = `users:${page}:${size}:${role ?? ''}:${q ?? ''}`
  return cachedDict(key, () => request<Paginated<UserOut>>(`/api/auth/users?${params.toString()}`))
}

/** 新建用户（仅 admin）。 */
export async function createUser(payload: UserCreate): Promise<UserOut> {
  invalidateDictPrefix('users:')
...
/** 角色列表（含权限集合）。 */
export async function getRoles(force = false): Promise<RoleOut[]> {
  if (force) invalidateDict('roles')
