"""错误事件 Pydantic 模型 + fire-and-forget 落库 helper。

错误管理页的数据来源：后端 HTTP 4xx/5xx（observability 中间件）与前端上报
（/api/events）都通过 capture_error 异步写入 error_event 表。写入走独立后台任务
+ 独立 session，best-effort：任何失败只告警不抛、绝不阻塞主请求。
"""
import asyncio
import logging
import time
